...
class RewardService
{
    ...
    ->select(
        ...
        // Conditional aggregate counts instead of subqueries
        DB::raw("COUNT(DISTINCT CASE 
            WHEN user_projects.status IS NOT NULL 
            AND user_projects.updated_at BETWEEN '{$startDate}' AND '{$endDate}' 
            THEN user_projects.id END) AS surveys_attempted"),

        DB::raw("COUNT(DISTINCT CASE 
            WHEN user_projects.status IN (1, 50) 
            AND user_projects.updated_at BETWEEN '{$startDate}' AND '{$endDate}' 
            THEN user_projects.id END) AS surveys_completed"),

        DB::raw("COUNT(DISTINCT activity_log.id) AS monthly_profiles_filled")
    )
    ->whereBetween('user_projects.created_at', [$startDate, $endDate])
    ->whereIn('users.country_code', $countryCodeArr)
    ->where([
    ...
    public function sendInvitationMail($country_code)
    {
        ...
        */
        ...
        $users = DB::table('users')
            ...
            ->get();
            \Log::error('User Data for Reward: ' . json_encode($users->toArray()));
    ...
    */
    ...
    public function getAwardWinnerMailHtml($award_month, $country_code)
    {
        $pastWinners = SJPanelMonthlyAward::leftJoin('awards', 'awards.id', '=', 'SJPanel_Monthly_award.award_type')
            ...
            ->whereIn('SJPanel_Monthly_award.award_type', [1, 2, 3])
            ->select('SJPanel_Monthly_award.points', 'SJPanel_Monthly_award.amount', 'awards.name')
            ->orderBy('SJPanel_Monthly_award.award_type', 'desc')
            ->get()->toArray();
