...
class BasicDetailsProvided
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        */
        ...
        $languageCode = $getLang[0] ?? 'en'; // fallback to 'en'
        $countryCode = isset($getLang[1]) ? strtoupper($getLang[1]) : 'IN'; // fallback to 'IN'
        ...
        $baseQuery = DB::table('user_projects as up')
            ...
            ->where('p.survey_status_code', 'LIVE')
            ->where('up.user_id', $user_id)
            ->where('p.language_code', strtoupper($languageCode))
