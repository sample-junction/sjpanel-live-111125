...
class BasicDetailController extends Controller
{
     ...
     */
    protected $userRepository, $generalRepository, $countriesCurrenciesRepository;
    ...
    public function index()
    {
        ...
        */
        ...
        //}
            ...
            $userPoints = 0; // or any default value you prefer
			if ($user_points && isset($user_points->user_points['completed'])) {
			...
			//dd($active_user_add_data);
			//$fetch_user_achievement = $active_user_add_data->user_achievement;[24-04-2024]
            if ($active_user_add_data && isset($active_user_add_data->user_filled_profiles)) {
                $fetch_user_achievement = $active_user_add_data->user_filled_profiles;
            } else {
                $fetch_user_achievement = null; // or [] or a default structure
            }
        ...
        if($request->has('is_partial')){
            ...
            if (config('access.users.confirm_email')) {
                ...
                if(!$get_unsubscribe_details){
                    ...
                    $email = new UserConfirmation($user,$rand_code,$get_reffer_point,0,0);
                    Mail::send($email);                    
                }
            ...
            if(!empty($request->get('device_preference'))){
                if(in_array(1,$request->get('device_preference'))){
                    $device_preference = "2,3,4,1";
                }else{
                   $device_preference = implode(',',$request->get('device_preference')); 
                }
