...
class BlogController extends Controller
{
    ...
    public function getPosts(Request $request): View
    {
        ...
        $data = [
            'posts' => DB::connection('mysql_additional')->table('canvas_posts')
                ...
                ->where(function ($query) use ($countryCode) {
                    $query->whereRaw('JSON_CONTAINS(JSON_UNQUOTE(REPLACE(countries, \'""\', \'\')), ?)', ['"'.$countryCode.'"'])
        ...
        // $data = [
        //     'posts'  => DB::connection('mysql_additional')->table('canvas_posts')->whereIn('post_status',$status)->where('site_info','=','1')->orderByDesc('published_at')->simplePaginate(10),
        //     //(!$auth)?Post::published()->orderByDesc('published_at')->simplePaginate(10):
       ...
       if($countryCode!='US'){
        ...
        return view('frontend.blog.index')->with('ip',str_replace('.','-',request()->ip()))
        ->with('uuid',$uuid)
        ->with('dfiq',$DFIQ)
    ...
    public function findPostBySlug(string $slug,Request $request): View
    {
        ...
        //print_r(json_decode($data['meta'],true)['meta_description']);exit();
           ...
           if($countryCode!='US'){
            ...
            return view('frontend.blog.show')->with('ip',str_replace('.','-',request()->ip()))
            ->with('uuid',$uuid)
            ->with('dfiq',$DFIQ)
            ->withCountries($countries) 
            ->with('country_name',strtoupper($country))
