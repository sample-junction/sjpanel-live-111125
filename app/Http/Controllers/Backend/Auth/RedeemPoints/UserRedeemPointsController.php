...
class UserRedeemPointsController extends Controller
{
                                    ...
                                    if ($reminderdate[$i] != '' && $remindersent[$i] != '') {
                                        if ($remindersent[$i] != $req_data->reminder_count) {
                                            ...
                                            if ($update_reminder) {
                                                ...
                                                $reminder_sent = $this->notificationRepo->createNotification($req_data->user_uuid, 'Redeem Request', $req_data->id, $msg, 'Redeem Reminder');
    ...
    public function approveRequest(Request $request)
    {
        ...
        if ($update) {
            ...
            $update_notification = $this->notificationRepo->createNotification($user_uuid, 'Redeem Request', $redeem_request_id, $msg, 'Redemption Approved');
            $userToken = UserToken::where('user_id', $user->id)->value('device_token');
            $title = "SJ Panel";
            $body = strip_tags($msg);
            if (!empty($userToken)) {
    ...
    public function ribbonNotified(Request $request)
    {
        ...
        if ($update) {
            ...
            $rybbon_notified = $this->notificationRepo->createNotification($user_uuid, 'Redeem Request', $redeem_request_id, $msg, 'Redeem Status');
            $userToken = UserToken::where('user_id', $user->id)->value('device_token');
            $title = "SJ Panel";
            $body = strip_tags($msg);
            if (!empty($userToken)) {
    ...
    public function couponSend(Request $request)
    {
        ...
        if ($update) {
            ...
            $coupon_sent = $this->notificationRepo->createNotification($user_uuid, 'Redeem Request', $redeem_request_id, $msg, 'Redeem Status');
    ...
    public function couponRedeem(Request $request)
    {
        ...
        if ($update) {
            ...
            $coupon_redeemed = $this->notificationRepo->createNotification($user_uuid, 'Redeem Request', $redeem_request_id, $msg, 'Coupon Redeemed');
        ...
        if (!empty($id)) {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if ($change_status == 'approve') {
                    ...
                    $msg = __('frontend.notification_txt.redeem_rqst_coupon_redeem_1') . $red_rqst_val . '.';
                } else if ($change_status == 'coupon_lapsed') {
                    $msg = __('frontend.notification_txt.redeem_rqst_coupon_lapsed') . $red_rqst_val . '.';
                }
    ...
    public function couponLapsed(Request $request)
    {
        ...
        if ($update) {
            ...
            $coupon_redeemed = $this->notificationRepo->createNotification($user_uuid, 'Redeem Request', $redeem_request_id, $msg, 'Coupon lapsed');
            ...
            //event(new AfterRedeemApprove($user,$redeem_request));
            return \Redirect::back()
                ->withFlashSuccess("Coupon has been lapsed");
        } else {
            return \Redirect::back()
                ->withFlashDanger("Request is not yet Approved");
        }
