...
class UserController extends Controller
{
        ...
        ->filter(function ($item) {
            ...
            $formattedData = $paginatedData->map(function ($item) {
                return [
                    ...
                    'action' => $item['uuid'] ? 
                        '<a href="' . route('admin.auth.reports.survey_details', $item['uuid']) . '">
                            <i class="fa fa-eye" title="View"></i>
                        </a>' .
                        (!empty($item['panelist_id']) ? 
    ...
    public function userFraud(ManageUserRequest $request)
    {
        ...
        // }else{
            ...
            if($panelists_ids){
                $activeUsers->whereIn('users.panellist_id', $panelists_ids);
            }
            $activeUsers1 = $activeUsers
                ->groupBy('users.id')
    ...
    {
        $unique_ip_check = Settings::where('key', 'unique_ip_check')->value('value') ?? '0';

        return view('backend.unique_ip_check.index', compact('unique_ip_check'));
    }
    public function postUniqueIPCheck(Request $request)
    {
        $ip_check = $request->input('unique_ip_check', '0');
        ...
        return redirect()->back()->with('flash_success', 'IP check enabled');
    }
...
"231014500001"];*/
       ...
       // exit;
           $activeUsers1 =DB::table("users_indian")->where('run_status',0)->limit(500)->get();
        $stateToRegionMapping='';
        foreach($activeUsers1 as $user)
        {
                $new_data = [];
    ...
    public function monthlyAward(ManageUserRequest $request)
    {
        ...
        foreach ($activeUsers1 as $userr) {
            $zipcode = $userr->zipcode;
            $country = $userr->country;
            $stateName = $userr->state ?? null;
            $cityName = $userr->city ?? null;
            $panellist_id = $userr->panellist_id;
            ...
            if (!empty($pro_filled_data)) {
                foreach ($pro_filled_data->user_answers as $value) {
                    if (!empty($value) && isset($value['profile_question_code']) && $value['profile_question_code'] === 'DMA_NAME') {
                        $cityFromAnswer = $value['selected_answer'][0] ?? null;
                        break; // No need to loop further once found
                    }
                }
            }

            // Use DMA_NAME city if available, else fallback to $userr->city
            $finalCity = $cityFromAnswer ?? $cityName;

            // Store the data
            $userData[$panellist_id] = [
                'state' => $stateName,
                'city' => $finalCity,
                'region' => '-', // If region is not provided in $userr
                'country' => $country,
            ];
...
foreach ($cityMap as $key => $value) {
    ...
    public function panelist_survey_detail(Request $request, $panelist_id)
    {
        ...
        $surveys = \DB::table('projects')
            ...
            ->leftjoin('invite_sent_details', 'projects.id', '=', 'invite_sent_details.project_id')
            ...
            ->orderBy('invite_sent_details.created_at', 'desc')
            ->groupBy(
                'projects.id',
    ...
    public function sendPullInvite(Request $request)
    {
        ...
        foreach ($projectIds as $projectId) {
            ...
            $email_data = [
                ...
                'subject' => __('inpanel.mail.survey.reminder_subject_new', [
                    'survey_points' => number_format($pointsData / $countryPoints, 2)
                ]),
            ];
        ...
        return response()->json([
            'status' => true,
            'message' => 'Reminder Sent Successfully for all selected projects',
        ]);
    }
