...
/**
    ...
    public function getredeemRequest($status = null, $email = null)
    {
        ...
        $redeemRequest = $redeemRequest->distinct()->get();
        $hasRedeemData = $redeemRequest->contains(function ($item) {
            return !empty($item->id); // request_redeems.id exists
        });
    ...
    {
        $redeemRequest = User::where('country_code','!=','AU')->whereHas('roles',function($query){$query->where('id',4);})
