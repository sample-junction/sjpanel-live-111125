...
class LiveListener
{
    ...
    public function handle(AfterStatusChanged $event)
    {
        ...
        if($apaceProject->client_code=='SJTC'){
            return;
        }
        ...
        $dedupedUsers = $this->getDedupeMatchedUsers($project->apace_project_code); // get deduped user by Himanshu 27-10-2025
        foreach ($matchingUsers as $uuid => $quotaId) {

            // code commented by Himanshu 27-10-2025
            //$tmp_user_role = User::where('uuid', $uuid)->first()->roles->pluck('id')->first();
            
            // code added by Himanshu 27-10-2025
            $user = User::where('uuid', $uuid)->first();
            $tmp_user_role= $user->roles->pluck('id')->first();

            \Log::info("user id :" . $user->id . ", dedupe arr ." . json_encode($dedupedUsers));
            if (in_array($user->id, $dedupedUsers))  continue; // skip deduped user by Himanshu 27-10-2025
            // end here
    ...
    // added getDedupeMatchedUsers by Himanshu 27-10-2025
    /* public function getDedupeMatchedUsers($dedupArr=[]){
        \Log::info("In getDedupeMatchedUsers .".json_encode($dedupArr));
        ...
        \Log::info("retun user from getDedupeMatchedUsers .".json_encode($userProjects));
        return $userProjects;
    }
    */
    
    // added getDedupeMatchedUsers by Himanshu 27-10-2025
    public function getDedupeMatchedUsers($apace_code)
    {
        $apaceProject = \DB::connection('mysql_apace')->table('projects')->leftJoin('project_dedupes', 'project_dedupes.id', '=', 'projects.survey_dedupe_list_id')
            ->select('project_dedupes.dedupe_data', 'project_dedupes.dedupe_status')->where('projects.code', $apace_code)->first();

        if (
            !$apaceProject ||
            empty($apaceProject->dedupe_data) ||
            empty($apaceProject->dedupe_status)
        ) {
            return [];
        }

        $dedupe_data = json_decode($apaceProject->dedupe_data, true);
        if (json_last_error() !== JSON_ERROR_NONE || !is_array($dedupe_data)) {
            \Log::warning("Invalid dedupe_data JSON for apace_code: {$apace_code}");
            return [];
        }

        $dedupe_codes = isset($dedupe_data['live']) ? array_values($dedupe_data['live']) : [];
        if (empty($dedupe_codes)) return [];

        \Log::info("Found dedupe codes", ['apace_code' => $apace_code, 'codes' => $dedupe_codes]);

        $projects = Project::whereIn('apace_project_code', $dedupe_codes)->pluck('id')->toArray();
        if (empty($projects)) return [];

        $userProjectsQuery = UserProject::whereIn('project_id', $projects);
        $dedupeStatus = $apaceProject->dedupe_status ?? null;
        ...
        \Log::info("Returning deduped users", [
            'apace_code' => $apace_code,
            'user_ids' => $userProjects,
        ]);
