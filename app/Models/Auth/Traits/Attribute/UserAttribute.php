 ...
 * @return string
 */
    public function getFraudButtonAttribute()
    {
        return '<a class="dropdown-item" style="cursor:pointer;" onclick="openPopup(\'' . $this->panellist_id . '\')">'
         . __('labels.backend.access.users.table.fraud', ['attribute' => $this]) . '</a>';
 ...
 * @return string
 */
    public function getDoiSendButtonAttribute()
    {
        return $this->confirmed == 0
        ? '<a class="dropdown-item" style="cursor:pointer;" href="' . route('admin.auth.doi-reminder', ['panelist_id' => $this->panellist_id]) . '">'. __('labels.backend.access.users.table.Doi_reminder', ['attribute' => $this]) .'</a>'
        : '';
     ...
     * @return string
     */
    public function getActionButtons2Attribute()
    {
        if ($this->trashed()) {
            return '
				<div class="btn-group" role="group" aria-label="'.__('labels.backend.access.users.user_actions').'">
     ...
     * @return string
     */
    public function getSocialButtons2Attribute()
    {
        $accounts = [];
        ...
        foreach ($this->providers as $social) {
            $accounts[] = '<a href="'.route(
                'admin.auth.user.social.unlink',
                    [$this, $social]
            ).'" data-toggle="tooltip" data-placement="top" title="'.__('buttons.backend.access.users.unlink').'" data-method="delete">'.$social->provider.'</a>';
        }
