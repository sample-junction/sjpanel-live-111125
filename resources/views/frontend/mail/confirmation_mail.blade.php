...
<body style="background-color:white; font-family: 'Roboto', sans-serif;">
    ...
    <!-- Header Section -->
      ...
      <a href="{{env('APP_URL')}}"><img   src="{{asset('img/email_temp/logo.png')}}" alt="" style="width:170px;height:34px;padding:15px;"></a>
    </div>
    ...
    <div style="width:560px;height:383px;margin:30px auto;background-color:rgba(244, 250, 255, 1); border-radius:13px; text-align:center;">
        ...
        <p style="color:rgba(89, 89, 89, 1); font-size:16px; margin:0px; padding-left: 30px; padding-right: 30px; ">  {{__('strings.emails.auth.confirmation.com_details_1')}}  {{$get_reffer_point}}{{__('strings.emails.auth.confirmation.com_details_2')}}</p><br>        
    @php
        if($user->is_social == 1){
            $route = route('frontend.auth.account.confirm', ['token' => $confirmation_code, 'is_social' => 1, 'is_campaign' => $campaign]);
        }else{
            $route = route('frontend.auth.account.confirm', ['token' => $confirmation_code, 'is_campaign' => $campaign,'is_mobileLink' => $mobileLink]);
        }
    @endphp      
   ...
   <!-- Body_part_2 -->
   <div style="width:560px;height:114px;margin:40px auto; border:1px solid white;background-color:rgba(255, 251, 243, 1); border-radius:13px; text-align:center; padding:30px auto; box-sizing:border-box; padding:0px;">
