...
<body style="background-color:white; font-family: 'Roboto', sans-serif;">
    ...
    <div style="max-width: 560px; margin: 30px auto; background-color: #f4faff; border-radius: 13px; text-align: center; overflow: hidden;">
        <img  src="{{asset('img/email_temp/reset.png')}}" alt="" style="height: 81px; width: 81px; margin-top: 30px; margin-bottom: 10px;">
        <p style="font-size: 24px; line-height: 32px; font-weight: bold; margin: 0; padding: 0 20px;">{{__('frontend.welcome_mail.salutation_2')}} {{$user->first_name}},</p>
        <p style="color: #595959; font-size: 16px; margin: 10px 0; padding: 0 20px;"> {{__('strings.emails.auth.password_cause_of_email_new1')}}</p>
        <a href="{{route('frontend.auth.password.reset.form', $token)}}" style="display: inline-block; padding: 10px 20px; margin: 15px 0; font-size: 16px; font-weight: bold; text-align: center; text-decoration: none; color: #ffffff; background-color: #006bde; border-radius: 6px;" target="_blank">{{__('buttons.emails.auth.reset_password')}}</a>
        <p style="color: #595959; font-size: 16px; margin: 0; padding: 0 20px; margin-bottom:10px;">{{__('strings.emails.auth.password_cause_of_email_new2')}}  <a style="text-decoration: none;" href = "mailto: support@sjpanel.com">support@sjpanel.com</a> {{__('strings.emails.auth.password_cause_of_email_new4')}}</p><br>
      
    </div>
    
      <!-- Body_part_2 -->
   <div style="width:560px;height:auto;margin:40px auto; border:1px solid white;background-color:rgba(255, 251, 243, 1); border-radius:13px; text-align:center; box-sizing:border-box; padding:0px;">
    <p style="font-size:16px;line-height:20px;">{{__('strings.emails.auth.password_cause_of_email_new3')}}</p>
    </div>
