...
<body style="background-color:white; font-family: 'Roboto', sans-serif;">
    ...
    <div style="width:560px;height:auto;margin:30px auto;background-color:rgba(244, 250, 255, 1); border-radius:13px; text-align:center;">
        <img src="{{asset('img/email_temp/confimation.png')}}" alt="" style="height:81px;width:81px;margin-top:30px;margin-bottom:10px;">
        <p style="font-size:30px;line-height:20px;font-weight:bold;padding:0px;"> {{__('strings.emails.auth.confirmation.dear')}} {{$user->first_name}},</p><br>
        <p style="color:rgba(89, 89, 89, 1); font-size:16px; padding-left:10px;padding-right:10px;"> {{__('frontend.welcome_mail.welcome1')}}</p>          
        <p style="color:rgba(89, 89, 89, 1); font-size:16px;padding-left:12px;padding-right:12px; padding-bottom:0px;margin-top: 0px;">{{__('frontend.welcome_mail.welcome2')}} {{$get_reffer_point}} {{__('frontend.welcome_mail.welcome3')}} </p><br>

        @php
            $route = route('frontend.auth.account.welcome', ['id'=>$user->id]);
        @endphp 
        @if($userLocale === 'hi_IN')

            <p style="color:rgba(89, 89, 89, 1); font-size:16px; padding-left:17px; padding-right:17px; padding-bottom:10px; margin-top:0px;">
                {{ __('frontend.welcome_mail.welcome8') }}
                <a style="text-decoration: none;" href="{{ $route }}">{{ __('frontend.welcome_mail.welcome7') }}</a>
                {{ __('frontend.welcome_mail.welcome9') }}
                {{ __('frontend.welcome_mail.welcome4') }} {{ $ref_points }}
                {{ __('frontend.welcome_mail.welcome6') }}
                <a href="mailto:support@sjpanel.com">support@sjpanel.com</a>.
            </p>
        @else
            <p style="color:rgba(89, 89, 89, 1); font-size:16px; padding-left:17px; padding-right:17px; padding-bottom:10px; margin-top:0px;">
                <a style="text-decoration: none;" href="{{ $route }}">{{ __('frontend.welcome_mail.welcome7') }}</a>
   ...
   <!-- Body_part_2 -->
   <div style="width:560px;height:auto;margin:40px auto; border:1px solid white;background-color:rgba(255, 251, 243, 1); border-radius:13px; text-align:center; padding:30px auto; box-sizing:border-box;">
      <p style="color:rgba(89, 89, 89, 1); font-size:16px;padding: 20px;">{{__('frontend.welcome_mail.welcome5')}}
