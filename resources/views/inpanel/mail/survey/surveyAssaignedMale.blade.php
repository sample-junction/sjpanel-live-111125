...
@endphp
    ...
    <div style="width:560px;margin:30px auto;background-color:rgba(244, 250, 255, 1); border-radius:13px; text-align:center;">
        ...
        <p style="font-size:30px;line-height:20px;font-weight:bold;padding:0px;padding-bottom: 10px;"> {{__('inpanel.SurveyMail.salutation')}} {{$user->first_name}},</p>
        <p style="color:rgba(89, 89, 89, 1); font-size:16px;padding-left: 10px;padding-right: 10px;">  
        @if($user_project_count > 1)
            {{__('inpanel.SurveyMail.content_1',['survey_count' => $user_project_count])}}<br>
            <a href="{{route('inpanel.survey.index')}}" style="display: inline-block; padding: 10px 20px; margin: 15px 0; font-size: 16px; font-weight: bold; text-align: center; text-decoration: none; color: #ffffff; background-color: #006bde; border-radius: 6px;" target="_blank">{{__('buttons.emails.auth.survey_dashboard')}}</a><br>
            {{__('inpanel.SurveyMail.content_2')}} 
        @else
            {{__('inpanel.SurveyMail.content_4',['survey_count' => $user_project_count])}}<br>
            <a href="{{route('inpanel.survey.index')}}" style="display: inline-block; padding: 10px 20px; margin: 15px 0; font-size: 16px; font-weight: bold; text-align: center; text-decoration: none; color: #ffffff; background-color: #006bde; border-radius: 6px;" target="_blank">{{__('buttons.emails.auth.survey_dashboard')}}</a><br>
