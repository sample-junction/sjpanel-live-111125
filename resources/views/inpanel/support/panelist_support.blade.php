...
<script>
	window.addEventListener('DOMContentLoaded', function() {
		...
		if (msg) {
			...
			toastr.success(msg);
			sessionStorage.removeItem('successMessage'); // remove so it doesn't repeat
		}
	});
			...
			success: function(response) {
				...
				try {
					if (response.status === 200) {
						...
						toastr.success("{{ __('inpanel.user.profile.preferences.preferences_menu.reopen_ticket_msg') }}");
