            ...
            <tbody>
                ...
                <tr>
                    <td style="text-align:center;width: 600px;border-left:1px solid rgba(233, 233, 233, 1);border-right:1px solid rgba(233, 233, 233, 1);">
                        <div style="margin: 20px auto;width:540px;background-color:rgba(244, 250, 255, 1); border-radius:13px;padding:10px 20px;">
                            <img src="{{asset('img/img_email/Redeemption_1.png')}}" alt="" style="height:81px;width:81px;margin-top:30p;">

                            <p style="font-size:30px;line-height:24px;font-weight:bold;padding:0px;margin:20px 0px">{{__('inpanel.mail.invitation.invite_freind_content_1')}} {{$user->first_name}},</p>
                            @php 
                            /* if($lang!='UK'){
                                if($request_redeem->redeem_points*config('app.points.metric.conversion') < 1){
                                $points = $request_redeem->redeem_points*config('app.points.metric.conversion')*100 . __('inpanel.dashboard.cents'); 
								}else{
									$points = $currencies['currency_logo']. $request_redeem->redeem_points*config('app.points.metric.conversion');
								}
							}else{
								if($request_redeem->redeem_points*config('app.points.metric.conversion') < 1){

									$points = number_format(($request_redeem->redeem_points/$countryPoints)*config('app.points.metric.conversion')*100,2) . $currencies['currency_denom_plural']; 
								}else{
									$points = $currencies['currency_logo']. number_format(($request_redeem->redeem_points/$countryPoints),2);
								}
							} */
							
							if($request_redeem->redeem_points*$metricConversion < 1){
								$currency = ($request_redeem->redeem_points*$metricConversion*100 > 1) ? $currencies['currency_denom_plural'] : $currencies['currency_denom_singular'];
								$points = $request_redeem->redeem_points*$metricConversion*100 .' '. __($currency); 
							}else{
								$points = $currencies['currency_logo']. number_format($request_redeem->redeem_points*$metricConversion,2);
							}
                            
                            @endphp
                            <p style="color:rgba(89, 89, 89, 1); font-size:16px;line-height:24px; justify-content:center;margin:20px 0px">{{__('inpanel.mail.invitation.user_email_content_1')}}{{$points}} ({{$request_redeem->redeem_points}} {{__('inpanel.dashboard.points')}}) {{__('inpanel.mail.invitation.user_email_content_2')}} <a href="mailto:support@sjpanel.com" style="text-decoration:none;">{{__('inpanel.mail.invitation.user_email_content_3')}}</a>{{__('inpanel.mail.invitation.user_email_content_4')}}</p>

                            <p style="color:rgba(89, 89, 89, 1); font-size:16px;line-height:24px; justify-content:center;margin:20px 0px">{{__('inpanel.mail.invitation.user_email_content_5')}}</p>
                            <p style="color:rgba(89, 89, 89, 1); font-size:16px;line-height:24px; justify-content:center;margin:20px 0px">{{str_replace('"','',__('inpanel.mail.invitation.user_email_content_6'))}}</p>
                ...
                <!-- <tr>
                    <td style="text-align:center;width: 600px;padding: -20px;border-left:1px solid rgba(233, 233, 233, 1);border-right:1px solid rgba(233, 233, 233, 1);">
                        <div style="margin:auto;width:560px;height:89px;background-color:rgba(255, 251, 243, 1); border-radius:13px;padding: 20px 10px; text-align: center;">
                            <p style="color:rgba(89, 89, 89, 1); font-size:15px; margin-top:20px">{{__('inpanel.mail.invitation.invite_freind_content_6')}} <a style="color:rgba(0, 107, 222, 1);" href="mailto:support@sjpanel.com">{{__('inpanel.mail.invitation.invite_freind_content_7')}}.</a></p> 
                        </div>
                    </td>
                </tr> -->
                ...
                <tr>
                    <td style="text-align:center;width: 600px; border: 1px solid rgba(233, 233, 233, 1);margin-top:30px!important;">
