...
@section('content')
    ...
    <div class="card mt-4">
        <div class="card-header">
            <div class="row">
                <div class="col-sm-10">
                <div><a class="btn btn-primary" href="{{route('admin.auth.support.birthdaysent')}}">Back</a></div><br>
                    <label>
                        <h4>
                            Create And Save Clone Campaign
                        </h4>
                    </label>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-12">
                   ...
                   @foreach($datas as $data)
                      ...
                      @endphp
                    <form action="{{route('admin.auth.create-clone')}}" method="POST">
                        @csrf

                        <div class="form-group mt-2">
                            <label for=""><h4>Campaign Name :</h4></label>
                            <input type="text" name="campaign_name" class="form-control" placeholder="Enter Template Name" value="">
                      </div>

                      <div class="form-group mt-2">
                            <label for=""><h4>Campaign Subject :</h4></label>
                            <input type="text" name="campaign_subject" class="form-control" placeholder="Enter Template Name" value="{{$data->campaign_subject}}">
                      </div>

                    <div class="form-group mt-2">
                        <input type="hidden" name="camp_id" value="{{$data->id}}">
                        <input type="hidden" name="template_id" id="template_id" value="{{$data->template_id}}">
                        <input type="hidden" name="template_type" id="template_type" value="{{$data->template_type}}">
                        <label for=""><h4>Template Name :</h4></label>
                        <input type="text" name="template_name" class="form-control" placeholder="Enter Template Name" value="{{$data->template_name}}" readonly>
                    </div>

                    <div class="form-group mt-2">
                        <label for=""><h4>Template Body Content :</h4></label>
                        <textarea name="campaign_content" class="content" id="email_content" cols="60" rows="60">{!!$data->campaign_content!!}</textarea>
                    </div>
                    ...
                    <span class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#GalleryModal">Gallery</span>

                     <div class="form-group mt-2">
                            <label for=""><h4>Campaign Amount :</h4></label>
                            <input type="text" name="campaign_amount" class="form-control"  value="{{$data->campaign_amount}}">
                      </div>


                      <div class="form-group mt-2">
                            <label for=""><h4>Campaign type :</h4></label>
                            <input type="text" name="type" class="form-control"  value="{{$data->type}}">
                      </div>


                    <div class="form-group mt-2">
                        <label for=""><h4>Campaign Link :</h4></label>
                        <input  type="text" name="campaign_link" class="form-control"  value="{{$data->campaign_link}}">
                    </div>

                    <div class="form-group mt-2">
                        <label for=""><h4>Campaign Status :</h4></label>
                        <input  type="text" name="campaign_status" class="form-control"  value="{{$data->campaign_status}}">
                    </div>
                    
                    <div class="form-group mt-2">
                        <label for=""><h4>Campaign Start Date :</h4></label>
                        <input  type="date" name="campaign_start_date" class="form-control"  value="{{$data->campaign_start_date}}">
                    </div>
