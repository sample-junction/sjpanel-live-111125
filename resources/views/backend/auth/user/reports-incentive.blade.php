                    ...
                    <table class="table" id="user_management">
                        ...
                        @foreach($get_redeem_data as $result)
                            @php 
                                ...
                                $total_earned = $result->user_points['redeemed_points'] + $result->user_points['completed'];
                                $metricConversion = 1 / $result->countryPoints;
                                $currencies = $result->currencies;
                            @endphp

                            @if($total_earned != 0 && $panellist_id != "")
                                @php
                                    $completedPoints = $result->user_points['completed'];
                                    $convertedAmount = $completedPoints * $metricConversion;
                                @endphp

                                @if($convertedAmount < 1)
                                    @php
                                        $convertedSmall = round($convertedAmount * 100);
                                        $currency = ($convertedSmall > 1)
                                            ? $currencies['currency_denom_plural']
                                            : $currencies['currency_denom_singular'];
                                        $currencywithvalue = $convertedSmall . ' ' . $currency;
                                    @endphp
                                @else
                                    @php
                                        $currencywithvalue = $currencies['currency_logo'] . number_format($convertedAmount, 2);
                                    @endphp
                                @endif
