...
<script>
    ...
    // Code updated by Vikas For Language Search(Starting 4/11/2025)
    ...
    $(document).ready(function () {
        ...
        // Handle custom search behavior
        $('#user_management_filter input').off('keyup').on('keyup', function () {
            var value = this.value.trim();
            var localeColumnIndex = 5; // Adjust if your locale column index changes
            var isEnglishLetters = /^[A-Za-z]+$/.test(value); // Only A–Z or a–z

            // Only if 1–2 English letters → search in locale column
            if (isEnglishLetters && value.length > 0 && value.length <= 2) {
                table.columns().search('');
                table.column(localeColumnIndex).search(value, true, false, true).draw();
            } 
    ...
    // Code updated by Vikas For Language Search(Ending 4/11/2025)
